...
int si5351_write(uint8_t reg, uint8_t value, int fd) {
    int n;
    char cmd_i2cWrite[16 + 2] = {'T', 0x03, SI5351_BUS_BASE_ADDR, reg,
                                 value}; /* T, LEN, 16bytes data */
    char response[32];
...
int si5351_read(uint8_t reg, uint8_t* value, int fd) {
    char cmd_i2cWrite[16 + 2] = {'T', 0x82, SI5351_BUS_BASE_ADDR,
                                 reg}; /* T, LEN, 16bytes data */
    char cmd_i2cRead[16 + 2] = {'R', SI5351_BUS_BASE_ADDR | 0x01,
                                0x01}; /* R, AR, LEN */
    char response[32];
    int n;
...
uint32_t pll_calc(uint32_t freq, struct Si5351RegSet* reg, int32_t correction) {
    ...
    ref_freq +=
        (uint32_t)((double)(correction / 10000000.0) * (double)ref_freq);
    ...
    c = 1;
    if (rfrac)
        rational_best_approximation(rfrac, denom, SI5351_PLL_B_MAX,
                                    SI5351_PLL_C_MAX, &b, &c);
...
uint32_t multisynth_recalc(uint32_t freq, uint32_t pll_freq,
    ...
    c = 1;
    if (rfrac)
        rational_best_approximation(rfrac, denom, SI5351_MULTISYNTH_B_MAX,
                                    SI5351_MULTISYNTH_C_MAX, &b, &c);
...
int main(int argc, char* argv[]) { /* 无需关注波特率 */
    ...
    if (argc != 3) {
        fprintf(stderr, "%s: <serial port> <output frequency in hertz>\n",
                argv[0]);
        fprintf(stderr,
                "\nThis program can access SI5351 through CH55x USB-I2C "
                "adapter.\nIt only accesses SI5351 through fixed address "
