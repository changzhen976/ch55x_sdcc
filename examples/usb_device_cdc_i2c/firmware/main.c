...
__xdata __at(0x0000)
    uint8_t Ep0Buffer[DEFAULT_ENDP0_SIZE]; //端点0 OUT&IN缓冲区，必须是偶地址
__xdata __at(0x0040) uint8_t Ep1Buffer[DEFAULT_ENDP1_SIZE]; //端点1上传缓冲区
__xdata __at(0x0080)
    uint8_t Ep2Buffer[2 * MAX_PACKET_SIZE]; //端点2 IN & OUT缓冲区,必须是偶地址
...
unsigned char __code SerDes[] = { //序列号字符串描述符
    ...
    '-',  0x00, '3', 0x00, '-', 0x00, '1', 0x00, '7', 0x00};
unsigned char __code Prod_Des[] = {
    //产品字符串描述符
    0x16, 0x03, 'U',  0x00, 'S',  0x00, 'B',  0x00, ' ',  0x00, 't',
    0x00, 'o',  0x00, ' ',  0x00, 'I',  0x00, '2',  0x00, 'C',  0x00,
};
unsigned char __code Manuf_Des[] = {
    0x18, 0x03, 'Z', 0x00, 'h', 0x00, 'i', 0x00, 'Y', 0x00, 'u', 0x00,
    'a',  0x00, 'n', 0x00, ' ', 0x00, 'W', 0x00, 'a', 0x00, 'n', 0x00};

//cdc参数
__xdata uint8_t LineCoding[7] = {
    0x00, 0xe1, 0x00, 0x00,
    0x00, 0x00, 0x08}; //初始化波特率为57600，1停止位，无校验，8数据位。
...
void jump_to_bootloader() {
    ...
    __asm LJMP BOOT_ADDR /* Jump to bootloader */
        __endasm;
    while (1)
        ;
}
    ...
    if (USBByteCount) //USB接收端点有数据
    {
        ...
        if (uart_rx_status == 0) {
            ...
            { /* Transmit I2C Data: T: <LEN>, 16bytes data, performing S, <AR>, <DAT>, E */
                ...
                i2c_error_no = 0;
                i2c_frame_len = 0;
            } else if (uart_data == 'R') { /* Recieve I2C Data: R<AR><LEN> */
                i2c_frame_rx_len = 0;
                uart_rx_status = 3;
                i2c_error_no = 0;
                i2c_frame_len = 0;
        ...
        } else if (uart_rx_status == 1) {     // 54	03	C0	02	53
