...
void main() {
    ...
    mDelaymS(
        5); //It is recommended to modify the main frequency with a slight delay to wait for the chip power supply to stabilize
    ...
    P1_DIR_PU &=
        0x0C; //All touch channels are set as floating input, and channels that are not used can be left unset
    TouchKeyQueryCyl2ms(); //TouchKey query cycle 2ms
    GetTouchKeyFree();     //Get sampling reference value
#if DE_PRINTF
    for (i = KEY_FIRST; i < (KEY_LAST + 1);
         i++) //Print sampling reference value
    {
    ...
    while (1) {
        if (KeyBuf) //key_buf is non-zero, indicating that a key press was detected
        {
            ...
            mDelaymS(
                100); //Delay is meaningless, simulate single-chip to do button processing
        }
        mDelaymS(
            100); //Delay is meaningless, imitating microcontroller to do other things
    }
#else
    while (1) {
        ...
        if (KeyBuf) //key_buf is non-zero, indicating that a key press was detected
        {
            ...
            KeyBuf = 0;               //Clear key press sign
            mDelaymS(
                20); //Delay is meaningless, simulate single-chip to do button processing
        }
