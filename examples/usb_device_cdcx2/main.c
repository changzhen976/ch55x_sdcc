...
#define uart_fifo_is_full(uart)                                                \
    ((((uart)->uart_fifo_w_idx - (uart)->uart_fifo_r_idx) &                    \
      UART_FIFO_SIZE_MASK) == UART_FIFO_SIZE_MASK)
#define uart_fifo_length(uart)                                                 \
    (((uart)->uart_fifo_w_idx - (uart)->uart_fifo_r_idx) & UART_FIFO_SIZE_MASK)

#define uart_fifo_put(uart, b)                                                 \
    do {                                                                       \
        if (!uart_fifo_is_full(uart)) {                                        \
            (uart)->uart_fifo[(uart)->uart_fifo_w_idx] = b;                    \
            (uart)->uart_fifo_w_idx =                                          \
                ((uart)->uart_fifo_w_idx + 1) & UART_FIFO_SIZE_MASK;           \
        }                                                                      \
    } while (0)

#define uart_fifo_get_without_check(uart, b)                                   \
    do {                                                                       \
        *(b) = (uart)->uart_fifo[(uart)->uart_fifo_r_idx];                     \
        (uart)->uart_fifo_r_idx =                                              \
            ((uart)->uart_fifo_r_idx + 1) & UART_FIFO_SIZE_MASK;               \
    } while (0)
#endif
...
#define mark_ep0_nothing_upload()                                              \
    do {                                                                       \
...
#endif
                } else if (usb_last_setup_req.wIndexL ==
                           2) { /* interface 2 is CDC1 */
                    cdc = &cdc1;
#ifndef CDC_LOOPBACK
...
void usb_irq_setup_handler(uint8_t ep) {
    ...
    if (ep == 0) {
        if (USB_RX_LEN == (sizeof(USB_SETUP_REQ))) {
            setup_transfing_size =
                ((uint16_t)req->wLengthH << 8) | req->wLengthL;
        ...
        if (usb_configuration) {
            ...
            if (!cdc0.upload_busy && cdc0.usb_rx_size > 0) {
                __xdata uint8_t *ep2_in = Ep2Buffer,
                                *ep2_out = Ep2Buffer + MAX_PACKET_SIZE;
            ...
            if (!cdc1.upload_busy && cdc1.usb_rx_size > 0) {
                __xdata uint8_t *ep3_in = Ep3Buffer,
                                *ep3_out = Ep3Buffer + MAX_PACKET_SIZE;
