...
extern __xdata _DevOnHubPort DevOnHubPort
...
uint8_t
EnableRootHubPort(); // 使能ROOT-HUB端口,相应的bUH_PORT_EN置1开启端口,设备断开可能导致返回失败
void SelectHubPort(
    uint8_t
...
uint8_t USBHostTransact(
    uint8_t endp_pid, uint8_t tog,
    uint16_t timeout); // endp_pid: 高4位是token_pid令牌, 低4位是端点地址
uint8_t HostCtrlTransfer(
    __xdata uint8_t* DataBuf,
    uint8_t*
        RetLen); // 执行控制传输,8字节请求码在pSetupReq中,DataBuf为可选的收发缓冲区
...
uint8_t
HubGetPortStatus(uint8_t HubPortIndex); // 查询HUB端口状态,返回在TxBuffer中
uint8_t HubSetPortFeature(uint8_t HubPortIndex,
                          uint8_t FeatureSelt); // 设置HUB端口特性
uint8_t HubClearPortFeature(uint8_t HubPortIndex,
                            uint8_t FeatureSelt); // 清除HUB端口特性
uint8_t CtrlGetXPrinterReport1(void);             //打印机类命令
uint8_t
AnalyzeHidIntEndp(__xdata uint8_t* buf,
                  uint8_t HubPortIndex); // 从描述符中分析出HID中断端点的地址
uint8_t AnalyzeBulkEndp(__xdata uint8_t* buf,
                        uint8_t HubPortIndex); //分析出批量端点
...
uint8_t
EnumHubPort(); // 枚举指定ROOT-HUB端口上的外部HUB集线器的各个端口,检查各端口有无连接或移除事件并初始化二级USB设备
