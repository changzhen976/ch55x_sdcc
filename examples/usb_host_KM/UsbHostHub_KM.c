...
__code uint8_t SetupGetDevDescr[] = {
    USB_REQ_TYP_IN, USB_GET_DESCRIPTOR,    0x00, USB_DESCR_TYP_DEVICE, 0x00,
    0x00,           sizeof(USB_DEV_DESCR), 0x00};
__code uint8_t SetupGetCfgDescr[] = {USB_REQ_TYP_IN, USB_GET_DESCRIPTOR,
                                     0x00,           USB_DESCR_TYP_CONFIG,
                                     0x00,           0x00,
                                     0x04,           0x00};
__code uint8_t SetupSetUsbAddr[] = {USB_REQ_TYP_OUT,
                                    USB_SET_ADDRESS,
                                    USB_DEVICE_ADDR,
                                    0x00,
                                    0x00,
                                    ...
                                    0x00,
                                    0x00};
__code uint8_t SetupSetUsbConfig[] = {
    USB_REQ_TYP_OUT, USB_SET_CONFIGURATION, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
__code uint8_t SetupSetUsbInterface[] = {USB_REQ_RECIP_INTERF,
                                         USB_SET_INTERFACE,
                                         0x00,
                                         0x00,
                                         0x00,
                                         ...
                                         0x00,
                                         0x00};
__code uint8_t SetupClrEndpStall[] = {USB_REQ_TYP_OUT | USB_REQ_RECIP_ENDP,
                                      USB_CLEAR_FEATURE,
                                      0x00,
                                      0x00,
                                      0x00,
                                      ...
                                      0x00,
                                      0x00};
__code uint8_t SetupGetHubDescr[] = {HUB_GET_HUB_DESCRIPTOR,
                                     HUB_GET_DESCRIPTOR,
                                     0x00,
                                     USB_DESCR_TYP_HUB,
                                     0x00,
                                     ...
                                     0x00};
__code uint8_t SetupSetHIDIdle[] = {0x21, HID_SET_IDLE, 0x00, 0x00,
                                    0x00, 0x00,         0x00, 0x00};
__code uint8_t SetupGetHIDDevReport[] = {
    0x81, USB_GET_DESCRIPTOR, 0x00, USB_DESCR_TYP_REPORT, 0x00, 0x00, 0xFF,
    0x00};
...
__code uint8_t TouchAOAMode[] = {0x40, 0x35, 0x00, 0x00,
                                 0x00, 0x00, 0x00, 0x00};
...
__code uint8_t StringID[] = {
    ...
    'W',  'C',  'H',  'U',  'A',  'R',  'T',  'D',  'e',  'm',
    'o',  0x00, //model name
    0x57, 0x43, 0x48, 0x20, 0x41, 0x63, 0x63, 0x65, 0x73, 0x73,
    0x6f, 0x72, 0x79, 0x20, 0x54, 0x65, 0x73, 0x74, 0x00, //description
    '1',  '.',  '0',  0x00,                               //version
    0x68, 0x74, 0x74, 0x70, 0x3a, 0x2f, 0x2f, 0x77, 0x63, 0x68,
    0x2e, 0x63, 0x6e, 0, //URI
    0x57, 0x43, 0x48, 0x41, 0x63, 0x63, 0x65, 0x73, 0x73, 0x6f,
    0x72, 0x79, 0x31, 0x00 //serial number
};
__code uint8_t SetStringID[] = {
    0x40, 0x34, 0x00, 0x00, 0x00, 0x00, 0x04, 0x00, 0x40, 0x34, 0x00, 0x00,
    0x01, 0x00, 12,   0x00, 0x40, 0x34, 0x00, 0x00, 0x02, 0x00, 19,   0x00,
    0x40, 0x34, 0x00, 0x00, 0x03, 0x00, 4,    0x00, 0x40, 0x34, 0x00, 0x00,
    0x04, 0x00, 0x0E, 0x00, 0x40, 0x34, 0x00, 0x00, 0x05, 0x00, 0x0E, 0x00};

__xdata uint8_t UsbDevEndp0Size; //* USB设备的端点0的最大包尺寸 */
__xdata __at(0x0000) uint8_t RxBuffer[MAX_PACKET_SIZE]; // IN, must even address
__xdata
    __at(0x0040) uint8_t TxBuffer[MAX_PACKET_SIZE]; // OUT, must even address

uint8_t Set_Port = 0;

__xdata _RootHubDev ThisUsbDev; //ROOT口
__xdata _DevOnHubPort DevOnHubPort
...
void main() {
    ...
    while (1) {
        ...
        /* 如果设备是鼠标 */
        loc = SearchTypeDevice(
            DEV_TYPE_MOUSE); // 在ROOT-HUB以及外部HUB各端口上搜索指定类型的设备所在的端口号
        if (loc != 0xFFFF) { // 找到了,如果有两个MOUSE如何处理?
            printf("Query Mouse @%04X\n", loc);
            i = (uint8_t)(loc >> 8);
            len = (uint8_t)loc;
            SelectHubPort(
                len); // 选择操作指定的ROOT-HUB端口,设置当前USB速度以及被操作设备的USB地址
            endp =
                len ? DevOnHubPort[len - 1].GpVar[0]
                    : ThisUsbDev.GpVar[0]; // 中断端点的地址,位7用于同步标志位
            if (endp & USB_ENDP_ADDR_MASK) { // 端点有效
                s = USBHostTransact(USB_PID_IN << 4 | endp & 0x7F,
        ...
        /* 如果设备是键盘 */
        loc = SearchTypeDevice(
            DEV_TYPE_KEYBOARD); // 在ROOT-HUB以及外部HUB各端口上搜索指定类型的设备所在的端口号
        ...
        if (TIN0 == 0) { //P10为低，开始打印
            ...
            TxBuffer[13] = 0x18;
            loc = SearchTypeDevice(
                USB_DEV_CLASS_PRINTER); // 在ROOT-HUB以及外部HUB各端口上搜索指定类型的设备所在的端口号
            if (loc != 0xFFFF) { // 找到了,如果有两个打印机如何处理?
                ...
                endp =
                    len ? DevOnHubPort[len - 1].GpVar[0]
                        : ThisUsbDev.GpVar[0]; // 端点的地址,位7用于同步标志位
                printf("%02X  ", endp);
                if (endp & USB_ENDP_ADDR_MASK) { // 端点有效
        ...
        if (loc != 0xFFFF) {    // 找到了
            ...
            for (k = 0; k != 4; k++) {
                //端点是否有效？
                endp = loc ? DevOnHubPort[loc - 1].GpVar[k]
                           : ThisUsbDev
                                 .GpVar[k]; // 中断端点的地址,位7用于同步标志位
                if ((endp & USB_ENDP_ADDR_MASK) == 0)
                    break;

                printf("endp: %02X\n", (uint16_t)endp);
                SelectHubPort(
                    loc); // 选择操作指定的ROOT-HUB端口,设置当前USB速度以及被操作设备的USB地址
                s = USBHostTransact(USB_PID_IN << 4 | endp & 0x7F,
                                    endp & 0x80 ? bUH_R_TOG | bUH_T_TOG : 0,
                                    0); // CH554传输事务,获取数据,NAK不重试
                if (s == ERR_SUCCESS) {
        ...
        if (loc != 0xFFFF) {                    // 找到了
            loc = (uint8_t)
                loc; //目前USBHOST.C仅支持ROOTHUB下的Android操作,不用分析loc
            ...
            if ((endp & USB_ENDP_ADDR_MASK) != 0) //端点有效
            {
                ...
                if (s == ERR_SUCCESS) {
                    ...
                    if (len) {
                        ...
                        UH_TX_LEN = len;
                        s = USBHostTransact(USB_PID_OUT << 4 | endp & 0x7F,
                                            endp & 0x80 ? bUH_R_TOG | bUH_T_TOG
                                                        : 0,
                                            0xffff); //无限次重试下传
                        if (s == ERR_SUCCESS) {
                            endp ^= 0x80;               // 同步标志翻转
