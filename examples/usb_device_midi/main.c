...
__xdata __at(0x0000) uint8_t Ep0Buffer
    [DEFAULT_ENDP0_SIZE]; // Endpoint 0 OUT & IN buffer, must be even address
__xdata __at(
...
__code uint8_t DevDesc[] = {
    ...
    0x86, 0x1a, 0x22, 0x57,
    0x00, 0x01, 0x01, 0x02,
    0x03, /* VID PID bString */
    0x01};

__code uint8_t CfgDesc[] = {
    0x09, 0x02, sizeof(CfgDesc) & 0xff, sizeof(CfgDesc) >> 8, 0x02, 0x01, 0x00,
    0x80, 0x32, //配置描述符（两个接口）
    //以下为接口0（音频接口）描述符
    0x09, 0x04, 0x00, 0x00, 0x00, 0x01, 0x01, 0x00,
    0x00, // USB Audio Class description, no endpoint
    //以下为功能描述符
    ...
    0x02, 0x04, 0x01, 0x01, 0x01, 0x00,
    //以下为两个端点的描述符
    0x07, 0x05, 0x02, 0x02, 0x40, 0x00, 0x00, 0x05, 0x25, 0x01, 0x01,
    0x01, /* EMB MIDI JACK = 1, AssocJACKID=1, OUT */
    0x07, 0x05, 0x82, 0x02, 0x40, 0x00, 0x00, 0x05, 0x25, 0x01, 0x01,
    0x03 /* EMB MIDI JACK = 1, AssocJACKID=3, IN */

};
/*字符串描述符*/
unsigned char __code LangDes[] = {0x04, 0x03, 0x09, 0x04}; //语言描述符
unsigned char __code SerDes[] = { // Serial number string descriptor
    0x14, 0x03, '2', 0x00, '0', 0x00, '1', 0x00, '8', 0x00,
    '-',  0x00, '3', 0x00, '-', 0x00, '2', 0x00, '7', 0x00};
unsigned char __code Prod_Des[] = {
    // Product string descriptor
    0x14, 0x03, 'C', 0x00, 'H', 0x00, '5', 0x00, '5', 0x00,
    'x',  0x00, 'M', 0x00, 'I', 0x00, 'D', 0x00, 'I', 0x00,
};
...
volatile __idata uint8_t Midi_Input_Point =
    0; //循环缓冲区写入指针，总线复位需要初始化为0
volatile __idata uint8_t Midi_Output_Point =
    0; //循环缓冲区取出指针，总线复位需要初始化为0
...
void USBDeviceCfg() {
    ...
    USB_CTRL |=
        ...
        bUC_DMA_EN; //USB设备和内部上拉使能,在中断期间中断标志未清除前自动返回NAK
...
void USBDeviceEndPointCfg() {
    ...
    UEP1_CTRL =
        bUEP_AUTO_TOG | UEP_T_RES_NAK; //端点1自动翻转同步标志位，IN事务返回NAK
    UEP0_DMA = (uint16_t)Ep0Buffer; //端点0数据传输地址
    UEP4_1_MOD = 0X40; //端点1上传缓冲区；端点0单64字节收发缓冲区
    UEP0_CTRL =
        UEP_R_RES_ACK | UEP_T_RES_NAK; //手动翻转，OUT事务返回ACK，IN事务返回NAK
}
#if 0
/*******************************************************************************
...
void DeviceInterrupt(void)
    __interrupt(INT_NO_USB) //USB中断服务程序,使用寄存器组1
{
    uint16_t len;
        ...
        case UIS_TOKEN_OUT | 2: //endpoint 3# 端点批量下传
            if (U_TOG_OK)       // 不同步的数据包将丢弃
            {
                ...
                UEP2_CTRL =
                    ...
                    UEP_R_RES_NAK; //收到一包数据就NAK，主函数处理完，由主函数修改响应方式
            }
            break;
        ...
        if (UsbConfig) {
            if (USBByteCount) //USB接收端点有数据
            {
                ...
                printf("R=%d, %02x %02x %02x %02x\r\n", USBByteCount,
                       Receive_Midi_Buf[0], Receive_Midi_Buf[1],
                       Receive_Midi_Buf[2], Receive_Midi_Buf[3]);
#endif
            ...
            if (!UpPoint2_Busy) //端点不繁忙（空闲后的第一包数据，只用作触发上传）
            {
                ...
                if (length > 0) {
                    ...
                    //	Midi_Output_Point = 0;
                    //写上传端点
                    memcpy(Ep2Buffer + MAX_PACKET_SIZE, Receive_Midi_Buf,
                           length);      //环回MIDI设备
                    UEP2_T_LEN = length; //预使用发送长度一定要清空
                    UEP2_CTRL =
                        UEP2_CTRL & ~MASK_UEP_T_RES | UEP_T_RES_ACK; //应答ACK
                    UpPoint2_Busy = 1;
                    length = 0;
                    //}
                }
