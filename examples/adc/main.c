...
void ADCInterrupt(void) __interrupt(
    INT_NO_ADC) //ADC interrupt service routine, using register set 1
{
...
void main() {
    ...
    ADCInit(
        0); //ADC clock configuration, 0 (96clk) 1 (384clk), ADC module is on
    ...
    while (1) {
        for (i = 0; i < 4; i++) {
            printf(
                "AIN%02x: ",
                (uint16_t)
                    i); //Print and display of ADC sampling channel for debugging
            ADC_ChannelSelect(
                i); //ADC sampling power on and channel setting, i (0-3) means sampling channel
            ADC_START =
                1; //Start sampling, enter interrupt when sampling is completed
            mDelayuS(
                30); //Wait for the acquisition to complete before switching to the next channel
        }
        mDelaymS(1000);
    }
#else
    ...
    while (1) // ADC query method
    {
        for (i = 0; i < 4; i++) {
            ...
            ADC_START =
                1; //Start sampling, enter interrupt when sampling is completed
            while (ADC_START) {
            }; //When ADC_START becomes 0, it means sampling is completed
            printf("DATA: %d \n", (uint16_t)ADC_DATA);
